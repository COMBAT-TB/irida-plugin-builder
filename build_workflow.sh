...
docker run --workdir /pipelines/$WORKFLOW_NAME --rm -v $(pwd):/pipelines quay.io/combattb/irida-builder:21.09.2 mvn clean install
...
if [ $jar_count -gt 1 ] ; then
  ...
  docker run --workdir /pipelines/$WORKFLOW_NAME --rm -v $(pwd):/pipelines quay.io/combattb/irida-builder:21.09.2 sh /insert_tool_file.sh $ga_file $workflow_dir $version $jar_filename
fi

cp $WORKFLOW_NAME/target/*.jar $DESTINATION
docker run --workdir /pipelines/$WORKFLOW_NAME --rm -v $(pwd):/pipelines quay.io/combattb/irida-builder:21.09.2 mvn clean
